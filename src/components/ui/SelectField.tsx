...
const SelectField = ({ label, register, error, icon, options, ...rest }: SelectFieldProps) => {
    ...
    return (
        <div className="relative w-full">
            ...
            <div
                className="mt-[1px] justify-between pr-3 w-full border border-[#E3E3E3] rounded-[8px] flex items-center cursor-pointer"
                 onMouseDown={(e) => {
            ...
            >
                <div className='flex items-center'>
                    ...
                    <input
                        ...
                        className={`text-[#121212] cursor-pointer pl-2 pr-2.5 py-2 text-[10px] placeholder:text-[#666D80] placeholder:text-[10px] bg-transparent w-full focus:outline-none`}
                        onClick={(e) => e.preventDefault()} // prevent input focus to avoid keyboard popup on mobile
                    />
                </div>

                <IoIosArrowDown className='text-[16px] text-[#666D80]' />
            </div>
            ...
            >
                <div className="w-full bg-white rounded">
                    {options.map((item) => (
                        <div
                            key={item}
                            className="hover:bg-[#F6F8FA] px-3 text-[12px] py-2 text-[#121212] cursor-pointer"
