...
const ApartmentListing = ({ location, guest, price, feature }: { location: string; price: string; guest: string; feature: string }) => {
  const [filteredApartments, setFilteredApartments] = useState<Accommodation[]>([])
  ...
  }, [fetchAccommodations, apartments.length]);
 ...
 useEffect(() => {
  ...
  const matchesLocationAndFilters = apartments.filter((i: Accommodation) =>
  ...
  if (!feature || feature === "All") {
    setFilteredApartments(matchesLocationAndFilters);
    setLoad(false); // ✅ Ensure loading stops immediately
    return;
  }
      ...
      try {
        ...
        if (Array.isArray(generalNode)) {
          const features = generalNode.map((item: any) => {
            const englishLabel = item?.libelle?.find(
              (l: any) => l?.$?.lang === 'en'
            );
  ...
  // ✅ Cleanup function to cancel async logic if dependency changes
  return () => {
    isCancelled = true;
  };
}, [apartments, location, price, guest, feature]);
    ...
    <section className='sm:mt-[100px] mt-[120px] mb-16 md:mb-12 mb-10'>
      ...
      <div className="container ">
            ...
            <div className="sm:w-auto w-full sm:block flex justify-end">
              <CountResults number={filteredApartments.length} />
            </div>
          </div>
      ...
      <div className="container">
        {load ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:mt-8 mt-6">
            {filteredApartments.map((item: Accommodation, index: number) => (
              <AccommodationCard
                key={index}
